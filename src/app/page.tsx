...
export default async function Home() {
  ...
  const email = settings?.email || 'info@lenyvron.com';
  const whatsappLink = settings?.whatsappLink || 'https://wa.me/972500000000?text=היי, אשמח לשמוע פרטים על עוגות';
  const instagramLink = settings?.instagramLink || 'https://instagram.com/lenyvron';
    ...
    <main className="min-h-screen overflow-hidden">
      ...
      >
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
          ...
          <p className="text-3xl md:text-4xl font-heading-he text-chocolate/80 mb-4 animate-fade-in-up animate-delay-100">
            {settings?.heroSubtitle || 'עוגות קונטידור מעוצבות'}
          </p>

          <p className="text-lg md:text-xl text-chocolate/60 max-w-2xl mx-auto mb-6 animate-fade-in-up animate-delay-200">
            {settings?.heroDescription || 'קו חלוקה ארצי | מחירי סיטונאות | שירות מקצועי לעסקים'}
          </p>
          ...
          <div className="flex gap-4 justify-center flex-wrap animate-fade-in-up animate-delay-300">
            <a href="#gallery" className="btn-primary">לגלריה</a>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <div key={index} className="glass-card rounded-3xl p-8 text-center hover:shadow-glow transition-all duration-300 hover:-translate-y-2">
                <div className="text-5xl mb-4">{feature.icon}</div>
                <h3 className="text-2xl font-heading-he text-chocolate mb-3">{feature.title}</h3>
      ...
      <section id="about" className="py-32 px-4 relative">
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
          <div className="text-center mb-16">
            <span className="text-gold text-sm tracking-[0.2em] mb-4 block">הסיפור שלנו</span>
            <h2 className="text-5xl md:text-6xl font-heading-he text-chocolate mb-4">אודותינו</h2>
          ...
          <div className="glass-card rounded-[2.5rem] p-12 md:p-16 text-center">
            <p className="text-xl md:text-2xl text-chocolate/80 leading-relaxed mb-8">
              {settings?.aboutText || 'Leny V Ron - משווק סיטונאי בלעדי של מוצרי קונדיטוריה איכותיים.'}
            </p>
            <p className="text-lg text-chocolate/60 leading-relaxed mb-8">
              {settings?.aboutDescription || 'אנחנו מפיצים מגוון רחב של עוגות, קינוחים ומאפים מעוצבים מהקונדיטוריות המובילות בארץ. קו חלוקה ארצי, מלאי זמין ומחירי סיטונאות מיוחדים לבתי קפה, מסעדות, קייטרינג ועסקים.'}
            </p>
            <div className="flex items-center justify-center gap-8 text-gold flex-wrap">
              <div className="text-center">
                <div className="text-4xl font-heading mb-1">{settings?.stat1Number || '100+'}</div>
                <div className="text-sm text-chocolate/50">{settings?.stat1Label || 'מוצרים בקטלוג'}</div>
              </div>
              <div className="w-[1px] h-12 bg-gold/30 hidden sm:block" />
              <div className="text-center">
                <div className="text-4xl font-heading mb-1">{settings?.stat2Number || '10+'}</div>
                <div className="text-sm text-chocolate/50">{settings?.stat2Label || 'שנות ניסיון'}</div>
              </div>
              <div className="w-[1px] h-12 bg-gold/30 hidden sm:block" />
              <div className="text-center">
      ...
      <section id="gallery" className="py-32 px-4 bg-background-dark relative">
        ...
        <div className="container mx-auto max-w-7xl relative z-10">
          <div className="text-center mb-16">
            <span className="text-gold text-sm tracking-[0.2em] mb-4 block">מה אנחנו מציעים</span>
            <h2 className="text-5xl md:text-6xl font-heading-he text-chocolate mb-4">הקטגוריות שלנו</h2>
            <div className="decorative-line" />
          </div>
          ...
          <div className="text-center mt-10">
            <a href={instagramLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-chocolate/60 hover:text-[#E4405F] text-sm transition-colors">
      ...
      <section id="contact" className="py-20 md:py-32 px-4 relative">
        ...
        <div className="container mx-auto max-w-2xl relative z-10">
          <div className="text-center mb-10">
            <span className="text-gold text-xs tracking-[0.2em] mb-3 block">נשמח לשמוע מכם</span>
            <h2 className="text-4xl md:text-5xl font-heading-he text-chocolate mb-3">צור קשר</h2>
            <div className="decorative-line" />
          </div>
          <div className="glass-card rounded-2xl p-6 md:p-10">
            <p className="text-center text-chocolate/60 text-sm mb-8">מעוניינים לפתוח חשבון לקוח? צרו איתנו קשר לפרטים ומחירון</p>
            <div className="flex flex-col sm:flex-row gap-3 mb-8">
              <a href={`tel:${phoneClean}`} className="flex-1 flex items-center gap-3 p-3 rounded-xl bg-chocolate/5 hover:bg-gold/10 transition-colors group">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm font-semibold text-chocolate truncate">{phone}</div>
                </div>
              </a>
              <a href={`mailto:${email}`} className="flex-1 flex items-center gap-3 p-3 rounded-xl bg-chocolate/5 hover:bg-gold/10 transition-colors group">
                <div className="w-10 h-10 rounded-full bg-gold/20 flex items-center justify-center shrink-0">
                  <svg className="w-4 h-4 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-sm font-semibold text-chocolate truncate">{email}</div>
                </div>
              </a>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <a href={`tel:${phoneClean}`} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-gold to-caramel text-white text-sm font-medium rounded-full hover:shadow-lg hover:-translate-y-0.5 transition-all">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                התקשרו עכשיו
              </a>
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-5 py-2.5 border border-[#25D366] text-[#25D366] text-sm font-medium rounded-full hover:bg-[#25D366] hover:text-white transition-all">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
